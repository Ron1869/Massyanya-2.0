## Краткое описание
Massyanya-2.0 — это усовершенствованная версия торгового бота, ориентированного на криптовалютный рынок. Проект предоставляет веб-интерфейс для управления ботом, настройки параметров торговли, анализа данных и взаимодействия через Telegram. Бот построен с использованием Python и интегрирует современные технологии анализа данных, машинного обучения и автоматизации торговли.
...
1. **Веб-интерфейс:**
   ...
   - Поддерживает страницы:
     ...
     - Настройка торговли (параметры бота, выбор логики торговли).
     - Чат для команд и отображения прогнозов.
     - История торговых операций.
     - Вывод логов и состояния бота.

1.1 **Telegram-бот:**
  - Удалённое управление.
  - Команды для включения торговли, получения прогнозов и проверки состояния.
    
1.2 **Анализ данных:**
  - Интеграция 4 стратегий для прогнозирования.
  - Использование LSTM-моделей для анализа временных рядов.
  - Интеграция данных из MinIO, Investpy и парсеров новостей.
...
### Анализ данных
1. **Источники данных:**
   - **Investpy:** Сбор данных с финансовых рынков, включая цены активов, объёмы и исторические данные.
   - **News_parsing_movies:** Анализ текстовых данных новостей для выявления рыночных настроений.
   - **MinIO:** Облачное хранилище для исторических данных и результатов парсинга.
   - **Binance API:** Получение текущих котировок, объёмов и информации по сделкам.

2. **Машинное обучение:**
   - Использование LSTM-моделей для анализа временных рядов и прогнозирования цен активов.
   - Обучение моделей на данных из MinIO и Investpy.
   - Генерация сигналов для торговли (покупка, продажа, удержание позиции).

3. **Объединение данных:**
   - Консолидация данных из всех источников.
   - Вычисление торговых индикаторов (например, RSI, MACD, скользящие средние).
   - Анализ корреляций между новостями и ценами активов.
...
1. Главные директории и их назначение

1.1 _assets

Содержит статические ресурсы, такие как изображения, логотипы, или файлы стилей, которые могут быть использованы в интерфейсе или документации.

1.2 .git

Хранит метаданные репозитория Git для отслеживания изменений кода. Не требует ручного изменения.

1.3 .github

Шаблоны и файлы для автоматизации CI/CD через GitHub Actions. Может включать:

workflows для тестирования или деплоя.

1.4 config

Файлы конфигурации проекта:

.env: содержит настройки API-ключей Binance, MinIO и других сервисов.

config.json: файл для настройки общих параметров проекта (например, параметры таймфрейма, лимитов торгов и др.).

1.5 frontend

Исходный код веб-интерфейса:

templates/: HTML-шаблоны для страниц веб-интерфейса.

static/: статические файлы (CSS, JS, изображения).

scripts/: JavaScript-файлы для реализации логики взаимодействия интерфейса с сервером.

1.6 backend

Серверная часть проекта:

api/: эндпоинты для работы с клиентской частью.

services/: обработка логики работы с биржами, парсерами и Telegram-ботом.

models/: алгоритмы предсказания и хранения данных.

routes.py: маршруты для работы с API.

1.7 parsers

Модули для сбора и обработки данных:

investpy/: парсер данных с Investing.com.

news_parser/: парсер для обработки финансовых новостей.

1.8 database

Содержит базы данных и утилиты для работы с ними:

db.sqlite3: основная база данных SQLite (может быть заменена на PostgreSQL/MySQL).

migrations/: миграции для управления структурой БД.

1.9 tests

Модули для тестирования:

Юнит-тесты для серверной и клиентской части.
...
error.log: запись ошибок.

trade.log: запись операций торговли.

forecast.log: запись данных прогнозов.

2. Ключевые файлы в корне проекта

2.1 README.md

Общая информация о проекте, его функциональности, настройке и запуске.
...
Скрипт для установки проекта и его зависимостей (если применяется).

2.4 docker-compose.yml

Конфигурация для запуска проекта в контейнерах Docker (если используется).

2.5 main.py

Точка входа для запуска серверной части проекта.

### Логика торговли
1. **Алгоритмы:**
   - Автоматическая обработка сигналов, полученных от LSTM и технического анализа.
   - Применение фильтров для управления рисками (например, максимальный объём сделки, стоп-лосс).
   - Учет фундаментальных факторов, таких как новостные события.

2. **Торговые стратегии:**
   - Возможность работы с несколькими шаблонами стратегий.
   - Управление выбором стратегии через веб-интерфейс.
   - Авто-переход между стратегиями на основе рыночных условий.

3. **Интерактивность:**
   - Пользователь может запускать и останавливать торговлю через интерфейс и Telegram.
   - Отображение результатов торговли в реальном времени.
   - Возможность ручного выбора активов и логики работы.
...
5. **Добавление логик торговли:**
   - Реализовать шаблоны торговых стратегий.
   - Возможность выбора стратегии из веб-интерфейса (раздел "Настройки торговли").
   - Шаблоны для трендовых стратегий, скальпинга, арбитража и других методов.
...
## Итог
Massyanya-2.0 — это мощный инструмент для автоматизированной торговли, способный адаптироваться к меняющимся рыночным условиям. Будущие доработки позволят расширить функционал и обеспечить удобство использования для трейдеров любого уровня.
